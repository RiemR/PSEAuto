...
public class Util {
	...
	public Connection getCon() {
		...
		if (fc == null)
			log(TAG + ": FacesContext is null.");
		else {
			Application app = fc.getApplication();
			if (app == null)
				log(TAG + ": Application is null.");
			else {
				con = app.evaluateExpressionGet(fc, "#{" + DATABASE_BEAN_NAME + ".con}", Connection.class);
				if (con == null)
					log(TAG + ": Connection is null.");
			}
		}
	...
	public Object getBean(String sBean) {
		...
		if (sBean != null) {
			...
			if (fc != null) {
				...
				System.err.println("FacesContext in getBean ist null!");
		}
		/*
		 * return FacesContext.getCurrentInstance().getApplication().
		 * evaluateExpressionGet( FacesContext.getCurrentInstance(), "#{" + sBean +"}",
		 * Object.class );
		 */

		/*
		 * FacesContext fc = FacesContext.getCurrentInstance(); Application app =
		 * fc.getApplication(); Object o = app.evaluateExpressionGet( fc, "#{" + sBean
	 ...
	 * @return out Verschlüsseltes Passwort oder unverschlüsselte Input-Parameter
	 *         bei Fehler
	 */
